...
class SimpleNN(Module):
    def __init__(self, input_width: int, input_height: int, output_size: int):
        ...
        self.conv1 = torch.nn.Conv2d(1, 128, 3, padding=1)
        self.conv2 = torch.nn.Conv2d(128, 128, 3, padding=1)
        self.conv3 = torch.nn.Conv2d(128, 64, 5, padding=2, groups=2)
        self.conv4 = torch.nn.Conv2d(64, 2, 7, padding=3, groups=2, bias=0)
...
train_dataset = Subset(dataset, range(3000))
test_dataset = Subset(dataset, range(3000, 3650))
...
for e in range(nb_epochs):
    ...
    id_data = 25
    with torch.no_grad():
        data = test_dataset[id_data:id_data+1]
        X = torch.tensor(data[0]).to(device, dtype=torch.float)
        Y = data[1]
