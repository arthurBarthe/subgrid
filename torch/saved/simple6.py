...
class Dataset_psi_s(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = np.hstack((self.sx[index+2], self.sy[index+2]))
        features = np.concatenate((self.psi[index], self.psi[index+1],
                                   self.psi[index+2]), axis=0)
...
class SimpleNN(Module):
    def __init__(self, input_width: int, input_height: int, output_size: int):
        ...
        self.conv1 = torch.nn.Conv2d(3, 64, 5, padding=2)
        self.conv2 = torch.nn.Conv2d(64, 64, 5, padding=2)
        self.conv3 = torch.nn.Conv2d(64, 32, 5, padding=2)
...
test_dataset = Subset(dataset, np.arange(3000, 3648))
...
for e in range(nb_epochs):
    ...
    mlflow.log_metric('train mse', running_loss, e)
